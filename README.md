# JarvisPi - ChatGPT assistant on a Raspberry Pi
![Wall-e](/img/wall-e3.png?raw=true)

## Acknowledgements

Based on OpenLexaPi : [ARDings's OpenLexaPi](https://github.com/ARDings/OpenLexaPi) and DaVinci : [DevMiser's DaVinci](https://github.com/DevMiser/DaVinci)
This project has been largely based on OpenLexaPi and DaVinci with a few tweaks for my needs. All credit goes to their creators for making this awesome project.

A real-time AI voice assistant running on a **Raspberry Pi** (either zero 2W or Pi 4 -- what I have), powered by the **OpenAI Realtime API**. Say a wake word to activate it, speak naturally, and it responds through a USB speaker. An animated robot face renders on an HDMI display.
...
The following steps are required to run JarvisGPT on a Raspberry Pi :
...
- Follow the other steps below for preparing your Raspberry Pi and downloading JarvisPi

### hardware

| Component | Details | Link | Infos |
|---|---|---|---|
| **Hardware** | Raspberry Pi Zero 2 W / Raspberry Pi 4/5 | [Adafruit](https://www.adafruit.com/product/4292) | The brain |
| **Audio Option A** *(version 2)* | Soundcore Mini — Bluetooth Speaker + Mic | - | With a bluetooth mic and speaker |
| **Audio Option B** *(version 2)* | USB Speaker Bar (plug-and-play) | - | With a speaker bar |
| **Audio Option C** *(version 1)* | USB Mic + USB speaker | [Mic](https://www.adafruit.com/product/3367) + [Speaker](https://www.adafruit.com/product/3369) | To be used in Jarvis case |
| **Case** *(version 1)* | 3D printed STL for version with LED eyes, no display | - | version with LED eyes, no display, in Jarvis enclosure |
...
JarvisPi is a python program that uses the following tools
...
This project can run on a **Raspberry Pi Zero 1.1** — single-core ARMv6 @ 1 GHz, 512 MB RAM, no GPU. Rendering at full 800×480 every frame would saturate the CPU and starve the audio pipeline. By rendering animated elements on a 200×120 surface and scaling 4× with `pygame.transform.scale`, pixels touched per frame are reduced to 6.25% of full resolution. Combined with dirty-flag rendering, the display thread consumes a negligible fraction of CPU.
...
cd
git clone https://github.com/obenchekroun/JarvisPi.git
cd JarvisPi
```
...
```bash
sudo apt update
sudo apt install -y pipewire pipewire-pulse fonts-nanum python3-pygame python3-pip pulseaudio-utils
```
...
### 4. Editing the bashrc to add local bin folder where are stored useful bin
```bash
sudo nano ~/.bashrc
```
...
Go to https://openai.com/, sign up and in Personal icon, click on View API Keys.
Then click on "+ Create new secret key" to create your new secret API key. Copy your API key and keep it in a secure location.
...
```bash
sudo systemctl enable jarvispi.service
sudo systemctl start jarvispi.service
```
